...
{
    public class VmPeopleMng_Mine : BaseViewModel
    {
            ...
            departmentLoadCompleted += (o, e) =>
                {
                    ...
                    string b = "不包含";
                    string c = "";
                    GetPersonInfoTableRia(a, c, c, a, a, b);
                };
            #endregion
        }
        ...
        private void QueryPersonInfo()
        {
            ...
            GetPersonInfoTableRia(Departs[Departs_SelectedIndex].Trim(), PersonName.Trim(), PersonWorkSn.Trim(),
        ...
        {
            var childWnd_AddPerson = new ChildWnd_OperatePerson_Mine(-1, ChildWndOptionMode.Add, PersonOperate_callback);
            ...
            if (delete_personIds.Count > 0)
            {
                MsgBoxWindow.MsgBox(
                    ...
                    (e) =>
                    {
                        ...
                        {
                            //通过ria向后台发送请求
                            BatchDeleteRia(delete_personIds.ToArray());
                        }
                    });
        ...
        private void BatchModify()
        {
            ...
            {
                ChildWnd_BatchModify_Mine childW = new ChildWnd_BatchModify_Mine(UserPersonInfos_Selected, PersonOperate_callback);
                childW.Show();
            }
            else
            {
        ...
        /// </summary>
        /// </summary>
        /// <param name="depart_Name">部门名称</param>
        ...
        private void GetPersonInfoTableRia(string depart_Name, string person_Name, string person_WorkSn,
            string iris_Status, string iris_Register, string child_DepartMode)
        {
            try
            {
                ...
                EntityQuery<UserPersonInfo> list = serviceDomDbAccess.GetPersonsInfoTableQuery(depart_Name, person_Name,
                ...
                lo.Completed += delegate
                {
                    ...
                    //UserPersonInfosBinding = new BaseViewModelCollection<UserPersonInfo>();
        ...
        private void BatchDeleteRia(int[] personIds)
        {
            try
            {
                Action<InvokeOperation<OptionInfo>> invokeCallBack = (op) =>
                {

                    //异步获取数据
                    if (op.IsComplete && op.Value.isSuccess )
                    {
                ...
                WaitingDialog.ShowWaiting();
                serviceDomDbAccess.BatchDeletePerson(personIds, invokeCallBack, null);
            }
            catch (Exception e)
            {
                string s = e.ToString();
        ...
        //选中全部人员或者取消选中
        public void SelectAllPerson(object sender)
        {
        ...
        public void SelectItems(object sender)
        {

            DataGrid grid = sender as DataGrid;

            if (grid == null || grid.CurrentColumn == null)
            {
                return;
            }

            if (grid.CurrentColumn.DisplayIndex == 0)
            {
                if (grid.SelectedItem != null)
                {
                    var obj = grid.SelectedItem as UserPersonInfo;
                    if (obj != null)
                    {
        ...
        {
            int personId = (((HyperlinkButton)sender).DataContext as UserPersonInfo).person_id;
            var childWnd_CheckPerson = new ChildWnd_OperatePerson_Mine(personId, ChildWndOptionMode.Check, PersonOperate_callback);
        ...
        {
            int personId = (((HyperlinkButton)sender).DataContext as UserPersonInfo).person_id;
            var childWnd_CheckPerson = new ChildWnd_OperatePerson_Mine(personId, ChildWndOptionMode.Modify, PersonOperate_callback);
            childWnd_CheckPerson.Show();
        }
        ...
        {
            int personId = (((HyperlinkButton)sender).DataContext as UserPersonInfo).person_id;
            var childWnd_CheckPerson = new ChildWnd_OperatePerson_Mine(personId, ChildWndOptionMode.Delete, PersonOperate_callback);
            childWnd_CheckPerson.Show();
        }
        ...
        public void Record(object sender)
        {
            ...
            var chidlWnd = new RecogInfo(pInfo);
            chidlWnd.Show();
        }

        //停用虹膜 by cty || 优化代码 by wz
        public void StopIris(object sender)
        {
            UserPersonInfo pInfo = (((HyperlinkButton)sender).DataContext as UserPersonInfo);
            ...
            if (pInfo != null)
            {
                UserPersonInfos_StopIris.Add(pInfo);
            }
