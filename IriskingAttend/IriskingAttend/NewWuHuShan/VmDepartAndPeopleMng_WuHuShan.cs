/*************************************************************************
** 文件名:   VmDepartAndPeopleMng_WuHuShan.cs
×× 主要类:   VmDepartAndPeopleMng_WuHuShan
**  
...
namespace IriskingAttend.ViewModel.PeopleViewModel
{
    ...
    public class VmDepartAndPeopleMng_WuHuShan : BaseViewModel
    {
        ...
        private AsyncTaskType _asyncTaskType = AsyncTaskType.None;
        ...
        /// 开始同步人员信息
        /// </summary>
        public DelegateCommand SyncPersonInfoCommand
        {
            get;
            set;
        }
        ...
        private bool _syncPersonInfoBtnEnable = true ;
        /// <summary>
        /// 同步人员按钮是否有用
        /// </summary>
         public bool SyncPersonInfoBtnEnable
        {
            get { return _syncPersonInfoBtnEnable; }
            set
            {
                _syncPersonInfoBtnEnable = value;
                this.OnPropertyChanged(() => SyncPersonInfoBtnEnable);
            }
        }
        ...
        private string _stopAsyncTaskBtnContent;
        /// <summary>
        /// 停止异步任务的按钮的内容
        /// </summary>
        public string StopAsyncTaskBtnContent
        {
            get { return _stopAsyncTaskBtnContent; }
            set
            {
                _stopAsyncTaskBtnContent = value;
                this.OnPropertyChanged(() => StopAsyncTaskBtnContent);
            }
        }

        private string _stopAsyncTaskTextContent;
        /// <summary>
        /// 异步任务的提示信息
        /// </summary>
        public string StopAsyncTaskTextContent
        {
            get { return _stopAsyncTaskTextContent; }
            set
            {
                _stopAsyncTaskTextContent = value;
                this.OnPropertyChanged(() => StopAsyncTaskTextContent);
            }
        }
        ...
        public VmDepartAndPeopleMng_WuHuShan()
        {
            string s1 = "001";
            string s2 = "";
            ...
            StopAsyncTaskCommand = new DelegateCommand(new Action(StopAsyncTask));
            ...
            SyncPersonInfoCommand = new DelegateCommand(new Action(StartSyncPersonInfo));
            ...
            _departmentLoadCompleted += (o, e) =>
                {
                    //界面初始化载入全部人员的信息
                    GetPersonInfoTableRia(AllDepartId, "", "", "全部", "全部", "不包含");
                };
            #endregion
        }
        ...
        private void AddRecogInDB(AddRecog recogDialog, UserPersonRecogLog recog)
        {
            ...
            _asyncTaskType = AsyncTaskType.BatchAddRecord; 
            //显示进度条
            IsAsyncTaskVisible = Visibility.Visible;
            AsyncTaskProgress = 0;
            StopAsyncTaskBtnContent = "取消批量添加识别记录";
            StopAsyncTaskTextContent = "批量添加识别记录中：";
        ...
        /// 开始人员同步
        /// </summary>
        private void StartSyncPersonInfo()
        {
             SyncPersonInfoBtnEnable = false;
             WaitingDialog.ShowWaiting("正在同步，请耐心等待！");
             Action<InvokeOperation<int>> callBack = CallBackHandleControl<int>.OnInvokeErrorCallBack;
             ServiceDomDbAcess.GetSever().StartSyncPerson(callBack, null);
             CallBackHandleControl<int>.m_sendValue = (count) =>
             {
                 WaitingDialog.HideWaiting();
                 //如果待同步信息条数大于0,构建同步人员信息的异步任务
                 if (count > 0)
                 {
                     SyncPersonInfo(count, 50);
                     _asyncTaskType = AsyncTaskType.SyncPersonInfo;
                     //显示进度条
                     IsAsyncTaskVisible = Visibility.Visible;
                     AsyncTaskProgress = 0;
                     StopAsyncTaskBtnContent = "取消人员同步";
                     StopAsyncTaskTextContent = "人员同步任务进行中：";
                     SyncPersonInfoBtnEnable = false;
                 }
                 else if (count == -99)
                 {
                     MsgBoxWindow.MsgBox("后台正在进行人员同步！",
                             MsgBoxWindow.MsgIcon.Information, MsgBoxWindow.MsgBtns.OK,
                             null);
                     SyncPersonInfoBtnEnable = true;
                 }
                 else if (count == -10)
                 {
                     MsgBoxWindow.MsgBox("连接定位卡数据库错误！",
                            MsgBoxWindow.MsgIcon.Error, MsgBoxWindow.MsgBtns.OK,
                            null);
                     SyncPersonInfoBtnEnable = true;
                 }
                 ...
                 {
                     MsgBoxWindow.MsgBox("连接虹膜考勤数据库错误！",
                            MsgBoxWindow.MsgIcon.Error, MsgBoxWindow.MsgBtns.OK,
                            null);
                     SyncPersonInfoBtnEnable = true;
                 }
        ...
        //进行人员同步
        private void SyncPersonInfo(int count,int countPerTask)
        {
            //构建异步任务执行器
            if (_runnerTask != null)
            {
            ...
            _runnerTask = new AsyncActionRunner(GetSyncPersonTask(count, countPerTask));
            //下面是运行完成后的处理
            _runnerTask.Completed += (obj, a) =>
            {
                TimeSpan spendTime = DateTime.Now - dateStart;
                //如果取消了操作，则不弹出对话框
                if (_asyncResultSyncPerson.tag == -1)
                {
                    //操作员日志 
                    VmOperatorLog.InsertOperatorLog(1, "取消同步人员信息", "耗时：" + spendTime.ToString(@"hh\:mm\:ss"), null);
                }
                else if (_asyncResultSyncPerson.isSuccess)
                {
                    MsgBoxWindow.MsgBox("同步人员信息成功！",
                        MsgBoxWindow.MsgIcon.Succeed, MsgBoxWindow.MsgBtns.OK);
                    //操作员日志 
                    VmOperatorLog.InsertOperatorLog(1, "同步人员信息", "耗时：" + spendTime.ToString(@"hh\:mm\:ss"), () =>
                    {
                        //刷新人员
                        _serviceDomDbAccess = new DomainServiceIriskingAttend();
                        GetPersonInfoTableRia(curDepartId, PersonName.Trim(), PersonWorkSn.Trim(),
                ...
                {
                    MsgBoxWindow.MsgBox("同步人员信息失败！",
                        MsgBoxWindow.MsgIcon.Error, MsgBoxWindow.MsgBtns.OK);
                    //操作员日志 
                    string option_info = _asyncResultSyncPerson.option_info.Replace('\'',' ');
                    VmOperatorLog.InsertOperatorLog(0, "同步人员信息", _asyncResultSyncPerson.option_info + "\r\n" +
                             "耗时：" + spendTime.ToString(@"hh\:mm\:ss"), null);
                }
                SyncPersonInfoBtnEnable = true;
                IsAsyncTaskVisible = Visibility.Collapsed;
                _runnerTask = null;
        ...
        {
            if (_asyncTaskType == AsyncTaskType.None)
            {
                return;
            }
            string info = "";
            if (_asyncTaskType == AsyncTaskType.BatchAddRecord)
            {
                info = "是否取消批量添加识别记录？";
            }
            else if (_asyncTaskType == AsyncTaskType.SyncPersonInfo)
            {
                info = "是否取消同步人员信息？";
            }
            MsgBoxWindow.MsgBox(
        ...
        public void CancelAsyncTask()
        {
            if (_runnerTask != null && _runnerTask.Stop)
            {
                ...
                if (_asyncTaskType == AsyncTaskType.SyncPersonInfo)
                {
                    //通知后台取消了同步操作
                    ServiceDomDbAcess.GetSever().CancelSyncPerson();
                    IsAsyncTaskVisible = Visibility.Collapsed;
                }
                else if (_asyncTaskType == AsyncTaskType.BatchAddRecord)
                {
                    _runnerTask.Stop = false;
                    IsAsyncTaskVisible = Visibility.Collapsed;
                }
        ...
        private int _asyncRusltAddRecord;   //异步任务的操作返回信息
        /// <summary>
        /// 构造批量修改人员属性的异步任务队列 
        /// </summary>
        /// <returns></returns>
        ...
        private OptionInfo _asyncResultSyncPerson;
        /// <summary>
        /// 构造从定位卡同步人员信息的异步任务队列
        /// </summary>
        ...
        /// <returns></returns>
        private List<IAsyncAction> GetSyncPersonTask(int count, int countPerTask)
        {
            List<IAsyncAction> tasks = new List<IAsyncAction>();
            ...
            {
                int curRow = i;
                int curDeltaRow = countPerTask;

                ///创建新任务
                var taskTemp = new AsyncAction("SyncPersonInfoTask" + i);
                ...
                taskTemp.SetAction(() =>
                {
                    ...
                    {
                        _asyncResultSyncPerson = o;
                        //表示当前任务完成了,执行下一个任务
                        taskTemp.OnCompleted();
                    };

                    ServiceDomDbAcess.GetSever().SyncPersonInfo(curRow, curDeltaRow, count, callBack, null);
                }, true);
                tasks.Add(taskTemp);
            }
