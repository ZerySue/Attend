...
namespace IriskingAttend.ViewModel.SystemViewModel
{
    public class VmDeviceItemMng : BaseViewModel
    {
        ...
        private bool IsDevicInfoValid(ref  List<DeviceInfo> devInfos )
        {
            ...
            devInfos = this._listStackPanel.GetContent(DevInfo.dev_sn, DevInfo.place);
        ...
        private void AddDeviceRia(List<DeviceInfo> devInfos, bool batch)
        {
            ...
            try
            {
                ...
                //调用后台进行添加设备
                _domSrvDbAccess.AddDevice(devSn,place,startTimes.ToArray(),devTypes.ToArray(), onInvokeErrCallBack, null);  
            }
            catch (Exception e)
            {
                ErrorWindow err = new ErrorWindow(e);
        ...
        private void ModifyDeviceRia(List<DeviceInfo> devInfos) 
        {
            ...
            try
            {
                ...
                WaitingDialog.ShowWaiting();
                //调用后台修改设备信息
                _domSrvDbAccess.ModifyDevice(devSn, place, startTimes.ToArray(), devTypes.ToArray(), onInvokeErrCallBack, null);
            }
            catch (Exception e)
            {
                ErrorWindow err = new ErrorWindow(e);
        ...
        {
            List<DeviceInfo> devInfos = this._listStackPanel.GetContent("", "");
            List<string> startTimes = new List<string>();
            List<int> devTypes = new List<int>();
            foreach (var item in devInfos)
            {
